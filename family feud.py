...
# The program provides a graphical interface for the game Family feud.
...
# Currently all gui texts are written.
...
def terminate_error(error_description):
    if messagebox.showerror("FAMILY FEUD", error_description):
        sys.exit()
...
filename = filedialog.askopenfilename(initialdir=current_dir, title="Select a data file", filetypes=(("csv files", "*.csv"), ("all files", "*.*")))

if filename == "":
    terminate_error("No file selected")

if filename[-4:] != ".csv":
    terminate_error("Wrong file format, it must be .csv")
...
programIcon = pygame.image.load("family feud.ico")
pygame.display.set_icon(programIcon)

# Create the second window
window1 = tkinter.Tk()
window1.title("Familiy Feud Control")
window1.iconbitmap("family feud.ico")
...
for i, round_answers in enumerate(game1.answers):
    ...
    round_button = tkinter.Button(round_tab, text="Start", command=lambda round=i: game1.round_init(round))
    round_button.grid(row=0, column=2, sticky="ew")

    l_start_button = tkinter.Button(round_tab, text="Left", command=lambda: game1.set_starting_team("L"))
    l_start_button.grid(row=1, column=1, sticky="ew")

    r_start_button = tkinter.Button(round_tab, text="Right", command=lambda: game1.set_starting_team("R"))
    r_start_button.grid(row=1, column=3, sticky="ew")

    l_strike_button = tkinter.Button(round_tab, text="Strike", command=lambda: game1.incorrect_answer("L"))
    l_strike_button.grid(row=2, column=1, sticky="ew")

    r_strike_button = tkinter.Button(round_tab, text="Strike", command=lambda: game1.incorrect_answer("R"))
...
score_button = tkinter.Button(score_tab, text="Total Points", command=game1.show_scores)
score_button.pack()
tabControl.add(score_tab, text="Punctuation")
...
for w in range(5):
    for t in range(2):
        ...
        final_button = tkinter.Button(final_tab, text="Look", command=lambda r=w, c=t, answer_txt=input_answer, point_txt=input_points: game1.show_final_answer(answer_txt, point_txt, r, c))
        input_answer.grid(row=w + 1, column=t * 3)
        input_points.grid(row=w + 1, column=t * 3 + 1)
        final_button.grid(row=w + 1, column=t * 3 + 2)

startfinal_button = tkinter.Button(final_tab, text="Fast Money", command=game1.init_final_round)
...
tabControl.add(final_tab, text="Bonus Round")

# Create SFX tab
sfx_tab = ttk.Frame(tabControl)

button1 = tkinter.Button(sfx_tab, text="Applause", command=lambda: pygame.mixer.Sound.play(bravo_sound))
button1.pack()
